...
const ModalCancelForm = ({ isOpen, onClose, orderId, handles }) => {
  ...
  const handleSubmit = async () => {
    console.log('CustomCancelButton: Submitting request for order', orderId)
    setLoading(true)
    try {
      // Enviar os dados para um endpoint REST customizado
      await axios.post('/_v/custom-cancel-endpoint', {
        orderId: orderId,
        requestedAt: new Date().toISOString(),
      }).catch(err => {
        console.warn('CustomCancelButton: REST endpoint failed or not found, but continuing as per requirements', err)
      })

      setSuccess(true)
      console.log('CustomCancelButton: Success! Redirecting in 3s...')

      setTimeout(() => {
        window.location.href = 'https://www.planetadobebe.com.br/institutional/fale-conosco'
      }, 3000)
    } catch (error) {
      console.error('CustomCancelButton: Error in handleSubmit', error)
    } finally {
      setLoading(false)
    }
  }
